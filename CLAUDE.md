...
- **Panel (ui/):** the New Batch tab has a Direction section (words, uploaded reference images — `PUT /api/client/{gym}/reference/{name}`, raw bytes, image by first bytes, kept in `brands/{gym}/references/`) and a Scene library card (status, Refresh scenes… → the `scenes-refresh` run, every draft with Approve / Reject with a reason). A directed batch's Run confirmation lists its drafted scenes; the server adds `--approve-scenes` only when the confirmed ids equal the drafts on disk
